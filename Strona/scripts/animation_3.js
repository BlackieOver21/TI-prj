...
function drawGasketCurve(x1, y1, x2, y2, x3, y3, x4, y4, level, iter, ratio, ctx) {
    if (level === 0) {
        ctx.beginPath();
        ctx.moveTo(x1, y1);
        ctx.lineTo(x2, y2);
        ...
        if ((iter%2)==0){
            ...
            drawGasketCurve(x1, y1, x12, y12, x13, y13, x2, y2, level - 1, iter+1, 0, ctx);
            drawGasketCurve(x2, y2, x22, y22, x23, y23, x3, y3, level - 1, iter, 0, ctx);
            drawGasketCurve(x3, y3, x32, y32, x33, y33, x4, y4, level - 1, iter+1, 0, ctx); 
        } else {
            //1 segment
            var midX = (x1 + x2) / 2;
            var midY = (y1 + y2) / 2;
...
// animateCurve();     -------- function to call to animate Curve
